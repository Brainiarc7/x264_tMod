...
static int x264_param_restrict_device( x264_param_t *param, int i_profile, const char *device )
{
    ...
    while( s )
    {
        ...
        else if( !strcasecmp( s, "psv" ) )
        {
            ...
            param->i_level_idc = ( param->i_level_idc < 0 ) ?
                                 X264_LEVEL_IDC_PSV : X264_MIN( param->i_level_idc, X264_LEVEL_IDC_PSV );
            /* PSV supports Level at most 3.1, and the vbv-maxrate/vbv-bufsize of L3.1 is 17500, which is far beyond PSV's capability */
