...
#define CL_DEVICE_BOARD_NAME_AMD                    0x4038
    ...
    if( status == CL_SUCCESS )
    {
        /* Southern Island GPUs have board names of the format: "AMD Radeon HD 7XXX Series" */
        if( !strncmp( boardname, "AMD Radeon HD ", 14 ) && !strcmp( boardname + 18, " Series") && atoi(boardname + 14) >= 7000 )
            *b_is_SI = 1;
        return 1;
    }
    ...
    const char * const tahiti_names[] = { "Tahiti", "Pitcairn", "Capeverde", "Bali", NULL };
...
int x264_opencl_init( x264_t *h )
{
    ...
    /* Select the first OpenCL platform that supports a GPU device that supports our
...
#define MAX_IMAGE_TYPES 100
            cl_uint count = 0;
            cl_image_format imageType[MAX_IMAGE_TYPES];
            clGetSupportedImageFormats( h->opencl.context, CL_MEM_READ_WRITE, CL_MEM_OBJECT_IMAGE2D, MAX_IMAGE_TYPES, imageType, &count );
            count = X264_MIN( count, MAX_IMAGE_TYPES );
