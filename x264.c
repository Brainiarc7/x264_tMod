...
#define H2 if(longhelp==2) printf
    ...
    H2( ""
        ...
        X264_VBV_MAXRATE_PSP, X264_VBV_BUFSIZE_PSP, X264_LEVEL_IDC_PSV / 10.,
    ...
    H2( "      --opts <integer>        Set level of writing options in SEI [%d]\n"
        ...
        "                                  - 2: write x264 information and options\n", defaults->i_opts_write );
    H2( "Muxer specific:\n" );
...
static int64_t print_status( int64_t i_start, int64_t i_previous, int i_frame, int i_frame_total, int64_t i_file, x264_param_t *param, int64_t last_ts )
{
    ...
    SetConsoleTitle( buf ); // always use old indicator for console title
